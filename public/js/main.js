...
$.get('/api/events', function (data) {
	...
	events.forEach(function (event) {
        ...
        `;
		L.marker(event.coords).addTo(map).bindPopup(popupContent).openPopup();
	});
});
...
$('#searchForm').on('submit', function (e) {
	...
	var filteredEvents = events.filter((event) => event.title.toLowerCase().includes(query));
	$('#eventList').empty();
	filteredEvents.forEach((event) => {
		$('#eventList').append(`
            <li class="list-group-item">
                ...
                <div>Reaktiot:
                    <span class="btn btn-sm btn-light mx-1" role="button">😊</span>
                    <span class="btn btn-sm btn-light mx-1" role="button">👍</span>
                    <span class="btn btn-sm btn-light mx-1" role="button">🎉</span>
                </div>
                <a class="btn btn-link event-link" data-id="${event.id}" data-toggle="modal" data-target="#eventDetailModal">Kommentit (${event.comments.length})</a>
            </li>
        `);
	});
});
